...
exports.main = async (event, context) => {
    ...
    if (searchContent.title === ''){
        var res =  await db.collection('lovewall').orderBy('time','desc').get()
    }else if(searchContent.title === false){
        var res =  await db.collection('lovewall').where({
            _openid:openid
    ...
    for(let article of articles){
        ...
        article_userinfo.push({
            lovewall:{...article,likeorNot:likeorNot},
            author:userinfo,
