...
exports.main = async (event, context) => {
    ...
    let res = await db.collection('lovewall').orderBy('time','desc').skip(skip).limit(limit).get()
    let lovewalls = res.data
    let lovewall_userinfo = []
    // 根据openid 获取 文章的点赞信息-- 判断该用户是否点赞
    let openid_like_list = (await db.collection('lovewallLikes').where({
    ...
    console.log(likeAuthor_openId_list);
    for(let lovewall of lovewalls){
        let openid = lovewall._openid
        let lovewallid = lovewall._id
        // 根据openid查询个人信息
        ...
        // 获取文章点赞信息
        let likelist = (await db.collection('lovewallLikes').where({
            articleId:lovewallid
        }).get()).data

        // 如果文章id 存在于 用户的点赞列表中，则在article 数组中点赞标记为ture
        let likeorNot = likeAuthor_openId_list.includes(lovewallid)
        

        lovewall_userinfo.push({
            lovewall:{...lovewall,likeorNot:likeorNot},
            author:userinfo,
